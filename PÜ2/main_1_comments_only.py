# UC 2.0

# UC 2.1 Einlesen der Daten
# Dateien öffnen und mit numpy array konvertieren
# Eingabe Dateien sind aufgrund der letzten leeren Zeile #181 ein String Datentyp. Dieser leere Eintrag muss gelöscht werden

# UC 2.2 Vorverarbeiten der Daten
# Daten werden auf Plausibilität überürüft
# Fehler beseitigen
# Aussortierung von Messwerten die auf einen Messfehler hindeuten (zu große Abweichung von Mittelwert)
# Richtige Daten als neues Array anlegen

# UC 2.3 Analysen der Daten auf Abbruchkriterium
# wenn Abbruchkriterium erreicht wurde, Daten aussortieren
# Falsche Daten seperat abspeichern um wahre und falsche Daten zu unterscheiden

# UC 2.4 Erstellen einer Zusammenfassung 
# Herausgefilterte Daten zusammenfassen
# Einen Überblick erstellen, welche Werte gefunden und benutzt wurden
# Visualisierung um Datenstand einfach abzulesen

# UC 2.5 Visualisierung der Daten
# einen Graphen mit matplotlib oder eine Textdatei erstellen
# Visualisierung variabel gestalten

# UC 2.6 Manuelle Eingabe eines Abbruchkriteriums
# Individuell bei Ausnahmen die Daten heraussortieren

# UC 2.7 Speichern der Daten
# Sicherung der fertigen Daten
# An Auftraggeber/Kunde weitergeben

# Gut! -YS
